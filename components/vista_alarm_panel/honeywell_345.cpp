...
HoneywellPacket honeywell345_parse(const rmt_symbol_word_t *symbols, size_t num_symbols) {
  ...
  size_t chip_count = 0;
  auto push_chip = [&](uint8_t c) {
    if (chip_count < kMaxChips)
      chips[chip_count++] = c;
  };
  ...
  for (size_t i = 0; i < num_symbols; i++) {
    ...
    for (int j = 0; j < n0; j++)
      push_chip(symbols[i].level0);

    int n1 = (symbols[i].duration1 >= threshold) ? 2 : 1;
    for (int j = 0; j < n1; j++)
      push_chip(symbols[i].level1);
  }
  ...
  for (size_t i = 0; i + 104 <= chip_count; /* i incremented inside */) {
    ...
    if (chips[i] != 0 || chips[i + 1] != 1 || chips[i + 2] != 0 || chips[i + 3] != 1 || chips[i + 4] != 0 ||
        chips[i + 5] != 1 || chips[i + 6] != 1 || chips[i + 7] != 0) {
      i++;
      continue;
    }
    ...
    if (valid_manchester) {
      ...
      bool match =
          (crc_calc_h == crc_rx || crc_calc_2g == crc_rx || crc_calc_h == crc_rx_le || crc_calc_2g == crc_rx_le);
      ...
      if (match) {
        ...
        pkt.serial = (uint32_t) (decoded[0] & 0x0F) << 16 | (uint32_t) decoded[1] << 8 | decoded[2];
        ...
        ESP_LOGV(TAG,
                 "CRC Mismatch at chip %zu: RX=0x%04X, Calc(H)=0x%04X, Calc(2G)=0x%04X [Data: %02X %02X %02X %02X]", i,
                 crc_rx, crc_calc_h, crc_calc_2g, decoded[0], decoded[1], decoded[2], decoded[3]);
      }
