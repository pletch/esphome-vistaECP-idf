...
// -----------------------------------------------------------------------
class VistaESPHome : public api::CustomAPIDevice, public time::RealTimeClock {
 public:
  // -------------------------------------------------------------------
  // Construction
  // -------------------------------------------------------------------
  ...
  void set_rf_heartbeat_external(bool external) { zones_.set_external_heartbeat_mode(external); }
  ...
  void init_cc1101(int mosi, int miso, int sck, int csn, int gdo0, int spi_bus) {
    ...
    cc1101_receiver_ = new CC1101Receiver(vistabus_, mosi, miso, sck, csn, gdo0, host);
  }

  void set_cc1101_rssi_threshold(int8_t dbm) {
    if (cc1101_receiver_)
      cc1101_receiver_->set_rssi_threshold(dbm);
  }
#endif
  ...
  void initialize_partition_sensors() { partitions_.initialize_sensor_vectors(); }
