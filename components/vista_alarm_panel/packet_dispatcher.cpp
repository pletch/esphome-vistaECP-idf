...
PacketDispatcher::PacketDispatcher(VistaBus &bus, const Config &cfg) : bus_(bus), cfg_(cfg) {}
...
void PacketDispatcher::dispatch_one() {
  ...
  if (cfg_.chksum_fail_sensor != nullptr && chksum_failures_ != chksum_last_reported_) {
...
void PacketDispatcher::note_chksum_failure_(const char *what) {
  if (esp_timer_get_time() < K_CHKSUM_SETTLE_US) {
    ESP_LOGI(TAG,
             ...
             what, static_cast<unsigned long long>(esp_timer_get_time() / 1000));
    return;
  }
  chksum_failures_++;
}
...
StatusFlags PacketDispatcher::decode_status_flags_(const char *cbuf, int size) {
  ...
  if (cfg_.partitions != nullptr) {
    for (const auto &p : cfg_.partitions->partitions()) {
      ...
      if (cbuf[(p.assigned_keypad >> 3) + 1] & (0x01 << (p.assigned_keypad & 0x07))) {
...
bool PacketDispatcher::assemble_legacy_se_(const char *payload, int size) {
  ...
  } else if (b0 == 0xFF && size == 5 && legacy_packet_index_ > 0 && legacy_packet_index_ < 8) {
...
void PacketDispatcher::print_packet_(const char *cbuf, int type, int src, int len) {
  ...
  for (int c = 0; c < len && pos + 4 < sizeof(hex); c++)
    pos += static_cast<size_t>(snprintf(hex + pos, sizeof(hex) - pos, "%02X ", static_cast<uint8_t>(cbuf[c])));
