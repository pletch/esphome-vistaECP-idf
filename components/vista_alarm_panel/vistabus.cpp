...
void VistaBus::rx_tx_task(void * args)
{
    ...
    while (1) 
    {
        // Handle any queued device msgs by sending F1 request  
        ...
        uint8_t buf[1];
        int rxBytes = vprotocol->handle_UART_events(pkt_to_send, buf);
        ...
        if (rxBytes) 
        {
            ...
            else if ( buf[0] == 0xF6) //SEND ACK Received
            {                 
                vprotocol->dispatchF6(pkt_to_send);
            }
            else if ( buf[0] == 0xF7 ) //DISPLAY
            {
                vprotocol->dispatchF7();
            }            
            else if ( buf[0] == 0xF8 ) //Unknown Device on vista20P. VistaSE uses to send display updates in program mode.
            {
                vprotocol->dispatchF8();
            }
            else if ( buf[0] == 0xF9 ) //LRR
            {   
                vprotocol->dispatchF9();
            }
            else if ( buf[0] == 0xFA && vprotocol->is_2400) //EXP
            {   
                vprotocol->dispatchFA();
            }
            else if ( buf[0] == 0xFB && vprotocol->is_2400) //5881EN traffic
            {    
        ...
        vprotocol->track_write_attempts(); //Track attempts to mark_pulse / write and fail if appropriate.
    }
...
{  
    ...
    while (1) 
    {
        ...
        int rxBytes = vprotocol->monitor_task_sync(buf, val);
        if (rxBytes) 
        {
