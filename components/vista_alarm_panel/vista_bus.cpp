...
void VistaBus::begin(int uartnum, int rxpin, int txpin, int extuartnum, int monitorpin) {
  ...
  xTaskCreate(rx_tx_task_start, "uart_rx_tx_task", kUartRxTxTaskStackSize, (void *) this, configMAX_PRIORITIES - 1,
              &this->rx_tx_task_Handle);
  ...
  if (this->monitor_pin != -1) {
    ...
    xTaskCreate(monitor_rx_task_start, "uart_monitor_rx_task", kUartMonitorTaskStackSize, (void *) this,
...
bool VistaBus::connected() const { return this->panel_connected; }

void VistaBus::emulateLRR(bool enabled) { LRRemulation = enabled; }
...
void VistaBus::init_uart(uart_port_t u_n, gpio_num_t rx_pin, gpio_num_t tx_pin) {
  const uart_config_t uart_config = {.baud_rate = static_cast<int>(kEcpBaudStandard),
...
void VistaBus::cache_baud_clock() {
  if (uart_set_baudrate(this->uart_num, kEcpBaudLegacy) != ESP_OK) {
    ESP_LOGW(TAG, "could not set %u baud; fast rate switching disabled", static_cast<unsigned>(kEcpBaudLegacy));
    uart_set_baudrate(this->uart_num, kEcpBaudStandard);
    return;
  }
  if (uart_set_baudrate(this->uart_num, kEcpBaudStandard) != ESP_OK) {
    ESP_LOGW(TAG, "could not restore %u baud; fast rate switching disabled", static_cast<unsigned>(kEcpBaudStandard));
    return;
  }
    ...
    if (uxQueueMessagesWaiting(deviceMsgQueue) && !vprotocol->req_to_send &&
      ...
      if (esp_timer_get_time() - q_msg.time < kDeviceMsgMaxWaitUs) {
        ...
        ESP_LOGE(TAG,
                 ...
                 "next reports.",
                 q_msg.address, static_cast<unsigned long long>(kDeviceMsgMaxWaitUs / 1000000));
      }
    }
