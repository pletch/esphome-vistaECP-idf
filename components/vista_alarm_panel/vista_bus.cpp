...
VistaBus::VistaBus() {
...
#ifndef LEGACY_SE_PROTOCOL
  vprotocol_ = std::make_unique<Vista20P>(*this);
#else
  vprotocol_ = std::make_unique<VistaSE>(*this);
#endif
  ...
  this->receiveQueue = xQueueCreate(K_RECEIVE_QUEUE_DEPTH, sizeof(ReceivedPacket));    // app reads decoded frames here
  this->sendQueue = xQueueCreate(K_SEND_QUEUE_DEPTH, sizeof(SendPacket));              // app posts commands here
  this->deviceMsgQueue = xQueueCreate(K_DEVICE_MSG_QUEUE_DEPTH, sizeof(DeviceMsg));    // expander/RF notifications
  this->rf_direct_queue = xQueueCreate(K_RF_DIRECT_QUEUE_DEPTH, sizeof(RfDirectMsg));  // fast direct-to-HA RF path

  this->rx_tx_task_Handle = nullptr;
  this->monitor_rx_task_Handle = nullptr;
  this->uartevtQueue = nullptr;
  this->panel_connected_ = false;
...
VistaBus::~VistaBus() {
  ...
  // vprotocol_ is a unique_ptr: it is destroyed automatically after this body
...
void VistaBus::begin(int uartnum, int rxpin, int txpin, int extuartnum, int monitorpin) {
  ...
  this->tx_pin_ = static_cast<gpio_num_t>(txpin);
  this->ext_uart_num = static_cast<uart_port_t>(extuartnum);
  this->monitor_pin_ = static_cast<gpio_num_t>(monitorpin);
  ...
  init_uart_(this->uart_num, this->rx_pin, this->tx_pin_);
  ...
  xTaskCreate(rx_tx_task_start, "uart_rx_tx_task", K_UART_RX_TX_TASK_STACK_SIZE, (void *) this,
              configMAX_PRIORITIES - 1, &this->rx_tx_task_Handle);
  ...
  if (this->monitor_pin_ != -1) {
    // tx_pin == -1 tells init_uart to install in RX-only mode.
    init_uart_(this->ext_uart_num, this->monitor_pin_, static_cast<gpio_num_t>(-1));
    xTaskCreate(monitor_rx_task_start, "uart_monitor_rx_task", K_UART_MONITOR_TASK_STACK_SIZE, (void *) this,
                configMAX_PRIORITIES - 10, &this->monitor_rx_task_Handle);
  }
}
...
bool VistaBus::connected() const { return this->panel_connected_; }

void VistaBus::emulate_lrr(bool enabled) { LRRemulation = enabled; }
...
void VistaBus::capture_pulse_pattern_(gpio_num_t rx_pin) {
  rmt_channel_handle_t rx_chan = nullptr;
  rmt_rx_channel_config_t rx_chan_config = {.gpio_num = rx_pin,
  ...
  rmt_receive_config_t receive_config = {.signal_range_min_ns = 2000,      // ignore glitches < 2 µs
  ...
  for (int i = 0; i < rx_data.num_symbols; i++) {
    ESP_LOGI(TAG, "Low(us): %05d  High(us): %05d", rx_data.received_symbols[i].duration0,
...
bool VistaBus::read_packet(char *data, int &len, int &type, int &src, bool with_delay) {
  ReceivedPacket pkt{};
  const bool result = with_delay ? xQueueReceive(this->receiveQueue, &pkt, portMAX_DELAY) == pdPASS
...
void VistaBus::init_uart_(uart_port_t u_n, gpio_num_t rx_pin, gpio_num_t tx_pin) {
  const uart_config_t uart_config = {.baud_rate = static_cast<int>(K_ECP_BAUD_STANDARD),
  ...
  if (static_cast<int>(tx_pin) == -1) {
    ESP_ERROR_CHECK(uart_driver_install(u_n, K_RX_BUF_SIZE + 8, 0, 0, nullptr, intr_alloc_flags));
  } else {
    ESP_ERROR_CHECK(
        uart_driver_install(u_n, K_RX_BUF_SIZE + 8, 0, K_UART_EVENT_QUEUE_DEPTH, &uartevtQueue, intr_alloc_flags));
  }
...
// this clock and leaves the port at K_ECP_BAUD_STANDARD, where init_uart() put it
...
void VistaBus::cache_baud_clock() {
  if (uart_set_baudrate(this->uart_num, K_ECP_BAUD_LEGACY) != ESP_OK) {
    ESP_LOGW(TAG, "could not set %u baud; fast rate switching disabled", static_cast<unsigned>(K_ECP_BAUD_LEGACY));
    uart_set_baudrate(this->uart_num, K_ECP_BAUD_STANDARD);
    return;
  }
  if (uart_set_baudrate(this->uart_num, K_ECP_BAUD_STANDARD) != ESP_OK) {
    ESP_LOGW(TAG, "could not restore %u baud; fast rate switching disabled",
             static_cast<unsigned>(K_ECP_BAUD_STANDARD));
    return;
  }

  uint32_t sclk_freq = 0;
  if (esp_clk_tree_src_get_freq_hz(static_cast<soc_module_clk_t>(UART_SCLK_DEFAULT),
                                   ESP_CLK_TREE_SRC_FREQ_PRECISION_CACHED, &sclk_freq) != ESP_OK ||
      sclk_freq == 0) {
  ...
  ESP_LOGD(TAG, "ECP baud switching armed: sclk=%lu Hz", static_cast<unsigned long>(sclk_freq));
}
...
void VistaBus::rx_tx_task_(void *args) {
  ...
  while (true) {
    ...
    // Rate-limit to one poll per K_PULSE_CYCLE_PERIOD ms to avoid bus
    // collisions with back-to-back polls.
    if (uxQueueMessagesWaiting(deviceMsgQueue) && !vprotocol_->req_to_send &&
        (esp_timer_get_time() - request_f1_time_ > K_PULSE_CYCLE_PERIOD * 1000)) {
      DeviceMsg q_msg;
      xQueuePeek(deviceMsgQueue, &q_msg, pdMS_TO_TICKS(0));
      if (esp_timer_get_time() - q_msg.time < K_DEVICE_MSG_MAX_WAIT_US) {
        request_f1_(q_msg.address);
        request_f1_time_ = esp_timer_get_time();
      } else {
        // Message has been waiting > 5 s with no F1 window — drop it.
        xQueueReceive(deviceMsgQueue, &q_msg, pdMS_TO_TICKS(0));
        ESP_LOGE(TAG,
                 "Dropping msg for device at address %d — no F1 window in %llu s. "
                 "The panel now holds a stale state for this device until it "
                 "next reports.",
                 q_msg.address, static_cast<unsigned long long>(K_DEVICE_MSG_MAX_WAIT_US / 1000000));
      }
    }
    ...
    if (this->stop_requested_ && monitor_rx_task_Handle == nullptr) {
      this->panel_connected_ = false;
      break;
    }
    ...
    if (now - last_data_received_ > 30LL * 1000 * 1000)
      this->panel_connected_ = false;
    ...
    if (now > 60LL * 1000 * 1000) {
      ...
      capture_pulse_pattern_(this->rx_pin);
      vTaskDelay(10);
      continue;
    }
#endif
    ...
    vprotocol_->check_send_q(pkt_to_send);
    ...
    uint8_t buf[1];
    const int rx_bytes = vprotocol_->handle_uart_events(pkt_to_send, buf);

    if (rx_bytes) {
      this->panel_connected_ = true;
      last_data_received_ = esp_timer_get_time();
      ...
      if (vprotocol_->req_to_send && vprotocol_->pulse_marked && pkt_to_send.type == 2)
        vprotocol_->req_to_send = false;
      ...
      if (buf[0] == 0xF2) {
        vprotocol_->dispatch_f2();  // keypad poll
      } else if (buf[0] == 0xF6) {
        vprotocol_->dispatch_f6(pkt_to_send);  // status / command packet
      } else if (buf[0] == 0xF7) {
        vprotocol_->dispatch_f7();  // long-range radio message
      } else if (buf[0] == 0xF8) {
        vprotocol_->dispatch_f8();  // zone-expander poll
      } else if (buf[0] == 0xF9) {
        vprotocol_->dispatch_f9();  // RF receiver poll
      } else if (buf[0] == 0xFA && vprotocol_->is_2400) {
        vprotocol_->dispatch_fa();  // 2400-series extended status
      } else if (buf[0] == 0xFB && vprotocol_->is_2400) {
        vprotocol_->dispatch_fb();  // 2400-series extended data
      } else if (vprotocol_->legacy_protocol && vprotocol_->is_2400) {
        vprotocol_->dispatch_legacy_status_packet(buf[0]);  // SE-series status
      } else if (buf[0] == 0) {
        // Single zero byte — discard silently.
      } else {
#ifdef DEBUG_LOG
        vprotocol_->dispatch_debug(buf[0], 0);
#endif
      }
    }

    // Track consecutive failed send attempts and log / back-off as needed.
    vprotocol_->track_write_attempts();
  }
...
// enough to keep pace.
void VistaBus::monitor_rx_task_(void *args) {
  uint8_t buf[1];
  ...
  while (true) {
    ...
    const int rx_bytes = vprotocol_->monitor_task_sync(buf, val);
    if (rx_bytes) {
      ...
      if (static_cast<uint8_t>(val >> 8) == 0xF6) {
        vprotocol_->dispatch_ext_f6(val, buf[0]);  // expander status
        val = 0;
      } else if (static_cast<uint8_t>(val >> 8) == 0xF8) {
        vprotocol_->dispatch_ext_f8(val, buf[0]);  // zone-expander response
        val = 0;
      } else if (static_cast<uint8_t>(val >> 16) == 0xF9) {
        vprotocol_->dispatch_ext_f9(val, buf[0]);  // RF receiver response
        val = 0;
      } else if (static_cast<uint8_t>(val >> 16) == 0xFA) {
        vprotocol_->dispatch_ext_fa(val, buf[0], vprotocol_->legacy_protocol);
        val = 0;
      } else if (static_cast<uint8_t>(val >> 16) == 0xFB) {
        vprotocol_->dispatch_ext_fb(val, buf[0]);
        val = 0;
      } else {
#ifdef DEBUG_LOG
        vprotocol_->dispatch_debug(buf[0], 1);
#endif
      }
    }
  }
...
static uint8_t get_expander_address(uint8_t zone) {
...
static uint8_t get_zone_bit(uint8_t zone, bool legacy_protocol) {
  if (legacy_protocol) {
...
VistaBus::EmulatedExpander *VistaBus::get_expander(uint8_t address) {
...
void VistaBus::request_f1_(uint8_t address) {
  SendPacket pkt{};
  pkt.type = 2;
  ...
  // and it is discarded after K_DEVICE_MSG_MAX_WAIT_US without an F1 window.  An AUI
  ...
  // req_to_send is clear and at most one per K_PULSE_CYCLE_PERIOD.
...
void VistaBus::set_zone_status_bit(uint8_t zone, bool open) {
  uint8_t address = 0;
  if (!vprotocol_->legacy_protocol) {
    address = get_expander_address(zone);
  } else {
    address = 1;  // legacy SE protocol uses a fixed expander address
  }

  EmulatedExpander *expander = get_expander(address);
  if (expander != nullptr) {
    const uint8_t bit = get_zone_bit(zone, vprotocol_->legacy_protocol);

    // Set or clear the status bit while leaving all other zone bits unchanged.
    expander->zone_status_bits = static_cast<char>(open ? (static_cast<uint8_t>(expander->zone_status_bits) | bit)
                                                        : (static_cast<uint8_t>(expander->zone_status_bits) & ~bit));

    // Notify the rx_tx_task that this expander needs an F1 poll.
    DeviceMsg exp_msg;
    exp_msg.address = address;
    exp_msg.source = zone;
    exp_msg.msg = static_cast<uint8_t>(open);
    exp_msg.time = esp_timer_get_time();
    ...
    if (xQueueSend(deviceMsgQueue, &exp_msg, 0) != pdPASS)
      ESP_LOGW(TAG, "Device message queue full; dropped expander notify for zone %d.", zone);
  }
}
...
void VistaBus::send_rf_msg(uint32_t serial, uint8_t msg) {
  DeviceMsg rf_msg;
  rf_msg.address = emulated_rf_receiver.address;
  rf_msg.source = serial;
  rf_msg.msg = msg;
  rf_msg.time = esp_timer_get_time();
  ...
  if (xQueueSend(deviceMsgQueue, &rf_msg, 0) != pdPASS)
    ESP_LOGE(TAG, "Device message queue full; dropped RF state for serial %" PRIu32 " (msg 0x%02X).", serial, msg);
}
...
void VistaBus::add_emulated_expander(uint8_t zone) {
  ...
  if (expander == nullptr) {
    ...
    new_expander.supervision_bits = static_cast<char>(static_cast<uint8_t>(new_expander.supervision_bits) & ~bit);
    ...
    expander->supervision_bits = static_cast<char>(static_cast<uint8_t>(expander->supervision_bits) & ~bit);
...
void VistaBus::emulate_rfr(uint8_t address) {
  RFRemulation = true;
...
void VistaBus::rx_tx_task_start(void *args) {
  ...
  tsk->rx_tx_task_(args);
}

void VistaBus::monitor_rx_task_start(void *args) {
  VistaBus *tsk = static_cast<VistaBus *>(args);
  tsk->monitor_rx_task_(args);
}
