...
void AUIManager::set_auto_sync(bool enabled) { clock_.auto_sync = enabled; }
...
void AUIManager::tick(VistaBus &bus, bool in_program_mode, time::RealTimeClock *rtc) {
...
void AUIManager::on_f2_packet(const char *payload, int size, ZoneManager &zones, time::RealTimeClock *rtc,
  ...
  while (n < static_cast<uint8_t>(payload[1] + 1) &&
         (static_cast<uint8_t>(payload[n]) == 0xFE || static_cast<uint8_t>(payload[n]) == 0xEC ||
...
void AUIManager::set_panel_time(VistaBus &bus, bool in_program_mode, time::RealTimeClock *rtc) {
  ...
  snprintf(&bytes[8], 14, "%02d%02d%02d%02d%02d%02d%d", t.year % 100, t.month % 100, t.day_of_month % 100, t.hour % 100,
           t.minute % 100, t.second % 100, (t.day_of_week + 9) % 10);
...
void AUIManager::handle_panel_time_response(const char *f2data, uint8_t data_len, time::RealTimeClock *rtc,
  ...
  const int64_t delta = llabs(static_cast<int64_t>(rtc_time.timestamp) - static_cast<int64_t>(panel_time.timestamp));
...
#ifdef DEBUG_LOG
void AUIManager::log_f2_type(uint8_t sum, uint8_t target, const char *f2data, uint8_t data_len) const {
  ...
  switch (sum) {
    case 1:
      ...
      break;
    case 2:
      type_str = "Program-Mode-Count";
      break;
    case 4:
      type_str = "All-Zones-Clear";
      break;
    case 20:
      type_str = "Panel-Time";
      break;
    case 21:
      ...
      break;
    case 22:
      type_str = "Device/Zone-Info";
      break;
    case 23:
      type_str = "Faulted-Zone(s)";
      break;
    default:
