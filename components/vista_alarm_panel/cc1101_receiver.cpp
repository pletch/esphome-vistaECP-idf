...
CC1101Receiver::CC1101Receiver(VistaBus &bus, int mosi, int miso, int sck, int csn, int gdo0,
                               spi_host_device_t spi_host)
    : bus_(bus), radio_(mosi, miso, sck, csn, gdo0, spi_host) {}

void CC1101Receiver::log_config() const {
  ESP_LOGCONFIG(TAG, "    MOSI: %d  MISO: %d  SCK: %d  CSN: %d  GDO0: %d", radio_.mosi_pin(), radio_.miso_pin(),
                radio_.sck_pin(), radio_.csn_pin(), radio_.gdo0_pin());
...
bool CC1101Receiver::begin() {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "rmt_new_rx_channel failed: %s — RF receiver disabled", esp_err_to_name(err));
    return false;
  }

  rmt_rx_event_callbacks_t cbs = {
      .on_recv_done = [](rmt_channel_handle_t, const rmt_rx_done_event_data_t *edata, void *user_data) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "rmt_rx_register_event_callbacks failed: %s — RF receiver disabled", esp_err_to_name(err));
...
void CC1101Receiver::check_health() {
  ...
  if (state != CC1101Status::MARCSTATE_RX) {
    ESP_LOGW(TAG, "Watchdog: MARCSTATE=0x%02X (expected 0x%02X) — kicking back to RX", state,
             CC1101Status::MARCSTATE_RX);
    radio_.kick_rx();
  }
}
