...
/*
The Vistabus class interfaces with the Ademco Vista Keybus via one (or optionally two) hardware UARTs on the ESP32
family devices. The primary RX / TX traffic (yellow / green wires) is collected in the first required UART via a
FreeRTOS Task that handles both data receipt and writing to the bus. If configured, a second FreeRTOS task is
instantiated to handle the second optional UART to receive additional data emitted to the bus on the TX (green wire)
associated with expansion devices such as RF wireless sensors.
...
class VistaBus {
 public:
  ...
  void suspend_monitor() {
    if (monitor_rx_task_Handle) {
      vTaskSuspend(monitor_rx_task_Handle);
      taskYIELD();
    }
  }
  void suspend_rx_tx() {
    if (rx_tx_task_Handle) {
      vTaskSuspend(rx_tx_task_Handle);
      taskYIELD();
    }
  }
  void resume_tasks() {
    if (rx_tx_task_Handle)
      vTaskResume(rx_tx_task_Handle);
    if (monitor_rx_task_Handle)
      vTaskResume(monitor_rx_task_Handle);
  }
  ...
  // and defer.
  bool device_msg_pending() const { return deviceMsgQueue != nullptr && uxQueueMessagesWaiting(deviceMsgQueue) > 0; }
  ...
  inline void set_baud_fast(uart_port_t port, bool standard) {
    ...
    HP_UART_SRC_CLK_ATOMIC() { uart_ll_set_baudrate(UART_LL_GET_HW(port), baud, baud_sclk_freq); }
