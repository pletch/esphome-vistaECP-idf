...
class AUIManager {
 public:
  ...
  void tick(VistaBus &bus, bool in_program_mode, time::RealTimeClock *rtc);
  ...
  void on_f2_packet_with_bus(const char *payload, int size, ZoneManager &zones, time::RealTimeClock *rtc,
                             VistaBus &bus);
  ...
  void request_time_sync(VistaBus &bus, bool in_program_mode, time::RealTimeClock *rtc);
...
#ifdef DEBUG_LOG
  void log_f2_type(uint8_t sum, uint8_t target, const char *f2data, uint8_t data_len) const;
#endif
