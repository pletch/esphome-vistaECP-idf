...
namespace esphome
{
    namespace alarm_panel
    {
        ...
        void PartitionManager::register_text_sensor(vistaECPTextSensor *sensor,
        ...
        void PartitionManager::register_status_sensor(vistaECPBinarySensor *sensor,
                                                      uint8_t partition_number,
                                                      const char *type)
        {
            if (partition_number == 0 || partition_number > partitions_.size())
            ...
            else if (strcmp(type, "ARMED_AWAY")   == 0) status_sensors_[idx].arma = sensor;
            else if (strcmp(type, "ARMED_STAY")   == 0) status_sensors_[idx].arms = sensor;
            else if (strcmp(type, "ARMED_INSTANT")== 0) status_sensors_[idx].armi = sensor;
            else if (strcmp(type, "ARMED_NIGHT")  == 0) status_sensors_[idx].armn = sensor;
            else if (strcmp(type, "ARMED")        == 0) status_sensors_[idx].arm  = sensor;
            else if (strcmp(type, "CHIME")        == 0) status_sensors_[idx].chm  = sensor;
            else if (strcmp(type, "ALARM")        == 0) status_sensors_[idx].alm  = sensor;
            else if (strcmp(type, "FIRE")         == 0) status_sensors_[idx].fire = sensor;
            else
        ...
        bool PartitionManager::get_partition_info(int partition_id,
                                                  uint8_t &keypad_addr,
        ...
        int PartitionManager::process_status_flags(const StatusFlags &flags,
                                                   ZoneManager &zones,
                                                   int64_t ttl)
        {
            ...
            const bool armed = flags.instant || flags.armed_away
                             || flags.armed_stay || flags.night;
        ...
        SysState PartitionManager::decode_system_state(const StatusFlags &flags,
        ...
        #define PUBLISH_BIN(sensor_ptr, cur_val, prev_val, force)       \
            do {                                                         \
                if (((cur_val) != (prev_val) || (force))                 \
                        && (sensor_ptr) != nullptr)                     \
                    (sensor_ptr)->process(cur_val);                     \
            } while (0)

        void PartitionManager::publish_light_states(size_t kpi,
                                                    const LightStates &cur,
                                                    const LightStates &prev,
                                                    bool force,
        ...
        void PartitionManager::update_display_lines(size_t kpi,
