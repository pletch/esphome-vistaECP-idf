      ...
      if (pkt.size > 1)  // split multi-byte payload into individual chars
      {
        ...
        for (int i = 0; i < pkt.size; i++) {
          ...
          if (xQueueSend(this->vistabus_.sendQueue, &next_char, pdMS_TO_TICKS(0)) != pdPASS) {
            ESP_LOGE(TAG,
                     "Send queue full — dropped character %d of %d. "
              ...
              if (this->req_to_send && pkt_to_send.type == 1 &&
                ...
                if (end - start > kBaudSwitchMarkMinUs && end - start < kBaudSwitchMarkMaxUs) {
                  ...
                  if (!this->legacy_programmode) {
                    ...
                    bytes = 0;
                    while (xQueueReceive(this->vistabus_.uartevtQueue, (void *) &pre_evt, pdMS_TO_TICKS(15)) ==
                           pdPASS) {
                    ...
                    uart_read_bytes_event(this->vistabus_.uart_num, buf, 1, pdMS_TO_TICKS(4),
                                          this->vistabus_.uartevtQueue);  // flush leading zero
                    bytes = uart_read_bytes_event(this->vistabus_.uart_num, buf, 1, pdMS_TO_TICKS(15),
