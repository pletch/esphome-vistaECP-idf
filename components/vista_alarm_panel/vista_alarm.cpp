...
struct VistaESPHome::OTAGuard : public ota::OTAGlobalStateListener {
  ...
  void on_ota_global_state(ota::OTAState state, float, uint8_t, ota::OTAComponent *) override {
    if (!owner)
      return;
...
void VistaESPHome::setup() {
  ...
  if (cc1101_receiver_ != nullptr && cc1101_receiver_->begin()) {
    ...
    cc1101_receiver_->set_packet_watchdog_enabled(zones_.has_physical_rf_zones());
...
void VistaESPHome::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  UART: %d  RX pin: %d  TX pin: %d", uart1_, rx_pin_, tx_pin_);
  if (monitor_pin_ != -1)
    ESP_LOGCONFIG(TAG, "  Monitor UART: %d  Monitor pin: %d", uart2_, monitor_pin_);
  ESP_LOGCONFIG(TAG, "  Default partition: %d", default_partition_);
  ESP_LOGCONFIG(TAG, "  Connection TTL: %lld s", ttl_ / (1000LL * 1000));
  ESP_LOGCONFIG(TAG, "  LRR supervisor: %s", lrr_supervisor_ ? "enabled" : "disabled");
  if (rfr_emulation_enabled_) {
    ESP_LOGCONFIG(TAG, "  RF receiver emulation: enabled  address: %d", rfr_emulation_addr_);
#ifdef CC1101_RECEIVER
  ...
  if (aui_.device_address() != 0) {
    ESP_LOGCONFIG(TAG, "  AUI address: %d  Auto clock sync: %s", aui_.device_address(),
  ...
  for (const auto &p : partitions_.partitions())
    ESP_LOGCONFIG(TAG, "    Partition %d  keypad address: %d", p.partition, p.assigned_keypad);
}
...
void VistaESPHome::rf_direct_task_start(void *args) { static_cast<VistaESPHome *>(args)->rf_direct_task(args); }

void VistaESPHome::rf_direct_task(void * /*args*/) {
  while (!stop_requested_) {
    RfDirectMsg msg;
    if (xQueueReceive(vistabus_.rf_direct_queue, &msg, pdMS_TO_TICKS(500)) == pdPASS) {
      if (!stop_requested_)
